...
if st.session_state["authentication_status"]:
    ...
    st.markdown(
        ...
        <div style='text-align:center; padding-top:20px;'>
            <h1 style='font-size:48px; margin-bottom:0;'>Dhammi Enterprise</h1>
    ...
    with st.form("property_form"):
        ...
        location = left.text_input("Name of Real Estate Location")
        exact_plot_number = right.text_input("Exact Plot Number")
        direction = right.text_input("Facing Direction of Real Estate")
        price_per_gaz = left.number_input("Price/Gaz", min_value=0)
        area_size_in_gaz = right.number_input("Area in Gaz", min_value=0)
        ...
        if use_geolocation:
            ...
            lat = left.number_input("Latitude", format="%.6f")
            lng = right.number_input("Longitude", format="%.6f")
    ...
    for property in properties:
        ...
        with col2:
            property_content = f"""
                ...
                <div>Exact Plot Number: {property.get('exact_plot_number', None)}</div>
                <div class=container>
                    <div class=box style='min-width:20%'>
                       <div class=h_1 >🏷 Price<br> </div>
                        <div class=h_1_value>{format_currency(property.get('price_per_gaz', 0), 'INR', locale='en_IN')}/Gaz </div>
                    </div>
                    <div class=box>
                        <div class=h_1> ⎕ Area<br> </div>
                        <div class=h_1_value>{format_decimal(property.get('area_size_in_gaz', 0), locale='en_IN')} Gaz </div>
                    </div>
                    <div class=box>
                        <div class=h_1>🟰 Total Costing<br></div>
                        <div class=h_1_value>{format_currency(property.get('price_per_gaz', 0) * property.get('area_size_in_gaz', 0), 'INR', locale='en_IN')}</div>
                    </div>
                </div>
                
                <div class=container style='gap:0px;'>
                    <div class=box>Description:<br>{property.get("description", None)}</div>
                    <div class=box>Geo Co-ordinates:<br>{property.get("latitude", None)}, {property.get("longitude", None)}</div>
                    <div class=box>Reference:<br>{property.get("reference", None)}</div>
                </div>
            """
            ...
            def edit(property):
                ...
                price_per_gaz = left.number_input("Price/Gaz", min_value=0, value=property.get("price_per_gaz", 0))
                area_size_in_gaz = right.number_input("Area in Gaz", min_value=0,
                ...
                lng = right.number_input("Longitude", format="%.6f", value=property.get("longitude", None))
                reference = st.text_input("Property Reference Person/Contact Details",
            ...
            @st.dialog("Do you want to delete the Estate?")
